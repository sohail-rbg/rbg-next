...
class ShaderProgram {
  ...
  init() {
    ...
    // Create shaders
    const vertexShader = this.createShader(gl.VERTEX_SHADER, this.options.vertex);
    const fragmentShader = this.createShader(gl.FRAGMENT_SHADER, this.options.fragment);
    ...
    if (this.options.depthTest === false) {
  ...
  resize() {
    ...
    if (this.options.onResize) {
      this.options.onResize.call(this, w, h, dpi);
    }
  ...
  updateBuffers() {
    ...
    if (this.buffers.position) {
      ...
      gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(this.buffers.position), gl.STATIC_DRAW);
    ...
    // Other buffers
    Object.keys(this.options.buffers).forEach(key => {
      if (this.buffers[key] && key !== 'position') {
        const buffer = gl.createBuffer();
        gl.bindBuffer(gl.ARRAY_BUFFER, buffer);
        gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(this.buffers[key]), gl.STATIC_DRAW);
        
        const location = gl.getAttribLocation(this.program, `a_${key}`);
        gl.enableVertexAttribArray(location);
        gl.vertexAttribPointer(location, this.options.buffers[key].size, gl.FLOAT, false, 0, 0);
      }
    });
  }
  ...
  animate() {
    ...
    if (this.options.onUpdate) {
      this.options.onUpdate.call(this, time);
    }
    
    // Set uniforms
    gl.useProgram(this.program);
    
    const projectionLocation = gl.getUniformLocation(this.program, 'u_projection');
    ...
    if (this.buffers.position) {
      gl.drawArrays(gl.POINTS, 0, this.buffers.position.length / 3);
    }
